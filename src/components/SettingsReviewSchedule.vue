    ...
    <b-list-group  v-if="settingsOpen" class="second-layer-settings" >
      <b-form>
        ...
        >
          <b-container>
            <b-row>
              <b-button @click="deleteInitialReviewStep()" class="btn-circle initial-schedule">
                ...
                required
              ></b-form-input>
              <b-form-input v-if="scheduleSettings.initialReviews.length > 1"
              class="initial-schedule"
                id="input-1-2"
                v-model="scheduleSettings.initialReviews[1]"
                type="number"
              ></b-form-input>
              <b-form-input v-if="scheduleSettings.initialReviews.length > 2"
                class="initial-schedule"
                id="input-1-3"
                v-model="scheduleSettings.initialReviews[2]"
                type="number"
              ></b-form-input>
              <b-form-input v-if="scheduleSettings.initialReviews.length > 3"
                class="initial-schedule"
                id="input-1-4"
                v-model="scheduleSettings.initialReviews[3]"
                type="number"
              ></b-form-input>
              <b-form-input v-if="scheduleSettings.initialReviews.length > 4"
                class="initial-schedule"
                id="input-1-5"
                v-model="scheduleSettings.initialReviews[4]"
                type="number"
              ></b-form-input>          
              <b-button  @click="addInitialReviewStep()" class="btn-circle initial-schedule">
        ...
        >
          <b-container>
            <b-row>
              <b-form-input style="max-width: 70%" type="range" v-model="scheduleSettings.laterReviewsMultiplier" min="1" max="3" step="0.05"></b-form-input>
              <div class="ml-4" style="font-weight: bold" >{{ scheduleSettings.laterReviewsMultiplier }}x</div>
            </b-row>
          </b-container>
        </b-form-group>
        <b-form-group
        ...
        >
          <b-container>
            <b-row>
              <b-form-input style="max-width: 70%" type="range" v-model="scheduleSettings.randomizer" min="0" max=".5" step="0.01"></b-form-input>
        ...
        >
          <b-form-select @input="failModeSelect(selected)" v-model="selected">
